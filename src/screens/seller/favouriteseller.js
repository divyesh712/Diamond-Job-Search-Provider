...
export default class Splash extends React.Component {
    ...
    async onUnsaveProductClick(productid){
        ...
        try {
            ...
            this.setState({paginationNumber:0,renderProducts:null,categories:null},() => {            
            this.getUserData();});
          }
          catch(error){
      ...
      return (
        ...
        {this.state.renderProducts == null ? 
            ...
            <View style={{alignItems:"center",justifyContent:"center",flex:1}}>
                <Text style={{color:'#4F45F0',fontSize:hp('3%')}}>{this.state.renderLanguage.noproductwishlisted}</Text>
            </View>

        :
        <View>
        <View style={{marginHorizontal:wp('5%'),flexDirection:'row',}}>
            <Text style={{color:'#000',fontSize:hp('2.5%'),fontWeight:"bold"}}>{this.state.renderLanguage.favourite}</Text>
        </View>
