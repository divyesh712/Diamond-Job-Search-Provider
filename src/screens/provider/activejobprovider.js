...
export default class Splash extends React.Component {
    ...
    async getUserData(){
        ...
        this.setState({userdata: value},()=>{
            ...
            this.ProviderActivePostApiCall(this.state.paginationNumber);
            this.setLanguage();
        })
    }
    ...
    async ProviderActivePostApiCall(pageno){
        ...
        try {
            ...
            if(this.state.myCustomAlert == 2){
                this.setState({myCustomAlert:0})
                this.dropDownAlertRef.alertWithType('success', this.state.renderLanguage.jobrecruitmentclose);
            }
    ...
    onEmployeeAppliedClick(postid){
        this.props.navigation.navigate('EmployeeApplied',{postID:postid});
    }

    onCloseRecruitmentClick(postid){
        Alert.alert(
            this.state.renderLanguage.closerecruitment,
            this.state.renderLanguage.wanttocloserecruit,
            [
              {text: this.state.renderLanguage.no, onPress: () => console.log('Cancel Pressed'), style: 'cancel'},
              {text: this.state.renderLanguage.yes, onPress: () =>  this.closeRecruitment(postid)},
            ],
            {cancelable: false},
          );
    }
      ...
      return (
        ...
        <View style={{flexDirection:'row',justifyContent:'space-between'}}>
            <View style={{marginTop:hp('2%'),marginLeft:hp('2.2%')}}>
                <Text style={{color:'white',fontSize:hp('2.5%')}}>{this.state.renderLanguage.activejobs}</Text>
            </View>
        ...
        {this.state.renderjobs == null ? 
            ...
            <FlatList
                        ...
                        <View style={{flexDirection:'row'}}>
                            <View style={{marginVertical:hp('2%'),marginLeft:hp('2%')}}>
                                        ...
                                        <View style={{marginVertical:hp('2%'),marginLeft:hp('3%'),width:hp('25%'),justifyContent:"center"}}> 
                                            <Text style={{fontSize:hp('2.1%'),fontWeight:'bold'}}>{item.category}</Text>
                                            <Text style={{marginTop:hp('1%')}}>{item.job_title}</Text>
                                        </View >
                        ...
                        <View style={{flexDirection:'row',marginTop:hp('1%'),alignItems:'center'}}>
                                ...
                                <View style={{flexDirection:'row',marginTop:hp('2%'),marginBottom:hp('0.1%'),marginLeft:hp('0.1%'),marginRight:hp('0.1%'),alignItems:'center',paddingVertical:hp('1%'),justifyContent:'space-between',backgroundColor:'#fff',borderRadius:hp('2%')}}>
                                    <TouchableOpacity onPress={()=>{
                                        //fontWeight:"900"
                                        if(item.total_appliedemp>0){this.onEmployeeAppliedClick(item._id)}
                                    }} 
                                        style={{flexDirection:'row',alignItems:'center'}}>
                                    <Text style={{marginLeft:hp('3%'),color:'#4F45F0',fontSize:hp('2%'),fontWeight:'900'}}>{this.state.renderLanguage.employeeapplied} ({item.total_appliedemp})</Text>
                                    </TouchableOpacity>
                                    ...
                                    <Text style={{marginRight:hp('3%'),color:'#4F45F0',fontSize:hp('2%'),fontWeight:'900'}}>{this.state.renderLanguage.closerecruitment}</Text>
                                    </TouchableOpacity>
